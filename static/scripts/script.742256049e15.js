...
function storeUtmParameters() {
  var utmParams = ['utm_source', 'utm_medium', 'utm_campaign', 'utm_content', 'utm_referrer', 'utm_url'];
  var eventData = {};
...
const buttonP2Request = [...document.getElementsByClassName('request-form-p2-readmore')]
let selectP2ElementRequest = [...document.getElementsByClassName('select-element-p2-request')]

if (selectP2ElementRequest.length) {
    selectP2ElementRequest.forEach(button => {
        button.addEventListener('click', () => {
...
if (buttonP2Request.length) document.addEventListener('click', e => {
    if (!buttonP2Request[0].contains(e.target)) {
        buttonP2Request.forEach(item => {
...
if (buttonP2Request.length) {
    buttonP2Request.forEach(button => {
        button.addEventListener('click', () => {
...
const requestPartnerForm2 = document.getElementById('request_partner_form_2')
const buttonsForOpenPartnerForm = document.querySelectorAll('[data-open-partner-form]');
const buttonsForOpenPartnerForm2 = document.querySelectorAll('[data-open-partner-form-2]');
const closePartnerForm = document.querySelector('[data-close-partner-form]');
const closePartnerForm2 = document.querySelector('[data-close-partner-form-2]');
const closePartnerFormMobile = document.querySelector('[data-close-form-partner-mobile]');
const closePartnerFormMobile2 = document.querySelector('[data-close-form-partner-2-mobile]');
...
if (requestPartnerForm2 && buttonsForOpenPartnerForm2.length && closePartnerForm2) {
    closePartnerForm2.addEventListener('click', () => {
        requestPartnerForm2.classList.add('hidden')
    })
    closePartnerFormMobile2.addEventListener('click', () => {
        requestPartnerForm2.classList.add('hidden')
    })
    buttonsForOpenPartnerForm2.forEach(button => {
        button.addEventListener('click', () => {
            if (requestPartnerForm2.classList.contains('hidden')) {
                requestPartnerForm2.classList.remove('hidden')
            } else {
                requestPartnerForm2.classList.add('hidden')
            }
        })
...
const formInputsPartner2Form = [...document.querySelectorAll('#request_partner_form_2 input')]

function checkPartner2Form() {
    const firstNameFilled = document.querySelector('#request_partner_form_2 input[name=firstname]').value.length > 0
        const lastNameFilled = document.querySelector('#request_partner_form_2 input[name=lastname]').value.length > 0
        // const titleFilled = document.querySelector('#request_partner_form #request-form-readmore_2 span').textContent.length > 0
        const phoneFilled = document.querySelector('#request_partner_form_2 input[name=phone]').value.length > 0
        const emailFilled = document.querySelector('#request_partner_form_2 input[name=email]').value.length > 0
        const companyFilled = document.querySelector('#request_partner_form_2 input[name=company]').value.length > 0
...
formInputsPartner2Form.forEach((item, i) => {
    item.addEventListener('input', (e) => {
        if (checkPartner2Form()){
            document.querySelector('#request_partner_form_2 button[name=send_form]').style.backgroundColor = 'white'
            document.querySelector('#request_partner_form_2 button[name=send_form_mobile]').style.backgroundColor = 'white'
        }
    })
})
...
function getUTMCookie(name)
    {
      ...
      if (match) {
        ...
        return match[2]
          ...
          return '{"utm_source":"","utm_medium":"","utm_campaign":"","utm_content":"","utm_referrer":"","utm_url":""}'
      }
   }
 ...
 if (requestForm) {
     requestForm.addEventListener('submit', (e) => {
         ...
         let utm_data = JSON.parse(getUTMCookie('FbAdUTMCookie'))
         requestForm.classList.add('hidden')
         ...
         xhr.send(JSON.stringify({
             custom_entity: {
                 external_values: {
                     ...
                     utm_referrer: utm_data['utm_referrer'],
                     utm_url: utm_data['utm_url']
                 }
             }
...
if (requestPartnerForm2) {
// Form partner handling
     requestPartnerForm2.addEventListener('submit', (e) => {
         let utm_data = JSON.parse(getUTMCookie('FbAdUTMCookie'))
         e.preventDefault(); // before the code
         requestPartnerForm2.classList.add('hidden')
         loader.classList.remove('hidden')
         ...
         xhr.send(JSON.stringify({
             easy_lead: {
                 first_name: document.querySelector('#request_partner_form_2 input[name=firstname]').value,
                 last_name: document.querySelector('#request_partner_form_2 input[name=lastname]').value,
                 job_title: document.querySelector('#request_partner_form_2 #request-form-p2-readmore span').textContent,
                 telephone: document.querySelector('#request_partner_form_2 input[name=phone]').value,
                 mobile_phone: document.querySelector('#request_partner_form_2 input[name=phone]').value,
                 email: document.querySelector('#request_partner_form_2 input[name=email]').value,
                 company_name: document.querySelector('#request_partner_form_2 input[name=company]').value,
                 value: '',
                 score: '',
                 source: '',
                 description: '',
